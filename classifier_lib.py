...
def get_grad_log_ratio(discriminator, vpsde, unnormalized_input, std_wve_t, img_resolution, time_min, time_max, class_labels, log=False):
    ...
    if tau.min() > time_max or tau.min() < time_min or discriminator == None:
        ...
        # print(unnormalized_input.shape)
        # mean_vp_tau = mean_vp_tau.unsqueeze(1).unsqueeze(2).unsqueeze(3)
        # print(mean_vp_tau.shape)
        # print(unnormalized_input.shape)
        input = mean_vp_tau.unsqueeze(1).unsqueeze(2).unsqueeze(3) * unnormalized_input
    with torch.enable_grad():
        ...
        discriminator_guidance_score *= - ((std_wve_t ** 2) * mean_vp_tau).unsqueeze(1).unsqueeze(2).unsqueeze(3)
    if log:
